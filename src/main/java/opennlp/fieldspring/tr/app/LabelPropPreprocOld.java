                    ...
                    if(toponym.getAmbiguity() > 0) {
                        ...
                        if(regionSet == null) {
                            ...
                            for(Location location : toponym.getCandidates()) {
                                //int regionNumber = TopoUtil.getCellNumbers(location, DEGREES_PER_REGION);
                                //regionSet.add(regionNumber);
                                regionSet.addAll(TopoUtil.getCellNumbers(location, DEGREES_PER_REGION));
                            }
    ...
    private static void writeRegionRegionEdges(String filename) throws Exception {
        ...
        for(int lon = 0; lon < 360 / DEGREES_PER_REGION; lon += DEGREES_PER_REGION) {
            for(int lat = 0; lat < 180 / DEGREES_PER_REGION; lat += DEGREES_PER_REGION) {
                int curRegionNumber = TopoUtil.getCellNumber(lat, lon, DEGREES_PER_REGION);
                int leftRegionNumber = TopoUtil.getCellNumber(lat, lon - DEGREES_PER_REGION, DEGREES_PER_REGION);
                int rightRegionNumber = TopoUtil.getCellNumber(lat, lon + DEGREES_PER_REGION, DEGREES_PER_REGION);
                int topRegionNumber = TopoUtil.getCellNumber(lat + DEGREES_PER_REGION, lon, DEGREES_PER_REGION);
                int bottomRegionNumber = TopoUtil.getCellNumber(lat - DEGREES_PER_REGION, lon, DEGREES_PER_REGION);
